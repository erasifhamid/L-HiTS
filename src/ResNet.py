...
class ResNet(torch.nn.Module):
    def __init__(self, arch, dt, step_size, activation=torch.nn.ReLU()):
        ...
        # device
        self.device = 'cuda' if torch.cuda.is_available() else 'cpu'           #this line is changed
        # self.device  = 'cpu'
        # layer
        self.activation = activation
        self.activation = activation
    ...
    def uni_scale_forecast(self, x_init, n_steps):
        ...
        # interpolations
        preds = torch.stack(preds, 2).cpu().detach().numpy()
...
def vectorized_multi_scale_forecast(x_init, n_steps, models):
    ...
    if n_steps==0:
        y_preds= torch.tensor(valid_preds).float()
    else:
